...
import {
  wellingtonVanTruckPages,
  getWellingtonSubPageBySlug,
...
const WellingtonVanTruckSubPage = ({ page: pageProp }: Props) => {
  const params = useParams<{ slug: string }>();
  const page = pageProp ?? (params.slug ? getWellingtonSubPageBySlug(params.slug) : undefined);

  if (!page) {
    return <Navigate to="/" replace />;
  }

  const hubPath = page.kind === 'van' ? '/van-hire-wellington' : '/truck-hire-wellington';
  const hubLabel = page.kind === 'van' ? 'Van Hire Wellington' : 'Truck Hire Wellington';
  const kindLabel = page.kind === 'van' ? 'Van Hire' : 'Truck Hire';
  ...
  const localBusinessLd = {
    ...
    areaServed: [page.locationName, 'Wellington', 'Lower Hutt', 'Porirua', 'Kāpiti Coast'],
    aggregateRating: { '@type': 'AggregateRating', ratingValue: '4.8', reviewCount: '1200' },
  };

  const related = wellingtonVanTruckPages
    .filter((p) => p.slug !== page.slug && p.kind === page.kind)
    .slice(0, 4);
    ...
    <div className="bg-background text-foreground">
      ...
      <nav aria-label="Breadcrumb" className="container mx-auto px-6 pt-6 text-xs text-muted-foreground">
        <ol className="flex flex-wrap items-center gap-2">
          ...
          <li>/</li>
          <li><Link to={hubPath} className="hover:text-primary">{hubLabel}</Link></li>
          <li>/</li>
        ...
        <div className="bg-[hsl(0_0%_92%)] text-[hsl(0_0%_8%)] flex flex-col justify-between p-8 md:p-16">
          ...
          <div className="max-w-xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-lg font-semibold">{kindLabel}</span>
            </div>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-4">
              ...
              <a href="tel:+6445551888" className="inline-flex items-center gap-2 text-sm font-semibold hover:text-primary">
                <Phone className="h-4 w-4" /> Call our Wellington branch
              </a>
            </div>
          </div>
        </div>
      ...
      <section className="container mx-auto px-6 py-16 md:py-20 max-w-4xl space-y-6">
        ...
        <div className="pt-6">
          ...
          >
            See the {page.recommendedName} <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </section>
      ...
      <section className="bg-[hsl(0_0%_92%)] text-[hsl(0_0%_8%)]">
        <div className="container mx-auto px-6 py-20">
          <h2 className="font-sans font-extrabold tracking-tight text-3xl md:text-4xl mb-12 max-w-2xl">
            What people use this {page.kind} for
          </h2>
      ...
      <section id="booking" className="border-t border-border scroll-mt-20">
        <div className="container mx-auto px-6 py-20 max-w-5xl">
          <div className="mb-10 text-center">
            <h2 className="font-sans font-extrabold tracking-tight text-3xl md:text-4xl">Book your {page.locationName} {page.kind}</h2>
            <p className="mt-4 text-muted-foreground">Live availability across our Wellington {page.kind} fleet.</p>
          </div>
          <SearchForm />
        </div>
      </section>
      ...
      {/* Related Wellington pages */}
      <section className="border-t border-border bg-[hsl(0_0%_18%)] text-[hsl(0_0%_92%)]">
        <div className="container mx-auto px-6 py-16">
          <h2 className="font-sans font-extrabold tracking-tight text-2xl md:text-3xl mb-8">
            More Wellington {kindLabel.toLowerCase()}
          </h2>
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-6">
            <Link to={hubPath} className="block p-5 rounded-xl bg-[hsl(0_0%_24%)] hover:bg-[hsl(0_0%_30%)] transition-colors">
              <p className="text-xs uppercase tracking-widest opacity-70">Main hub</p>
              <p className="mt-2 font-semibold">{hubLabel}</p>
            </Link>
            {related.map((r) => (
...
export default WellingtonVanTruckSubPage;